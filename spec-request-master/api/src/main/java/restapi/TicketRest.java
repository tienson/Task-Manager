        ...
        try {
            ...
            if (list_relater_id != null) {
                ...
                for (String relaterIDString : myList) {
                    ...
                    String[] str = new String[]{assignto.getEmail()};
                    emailService = new EmailService();
                    emailService.sendFromGMail(str, "thay đổi công việc", "thay đổi người liên quan của công việc :" + ticket.getSubject());
                }
    ...
    public Response updateAssignto(
        ...
        try {
            ...
            if (assigned_to > 0) {
                ...
                String[] str = new String[]{assignto.getEmail()};
                emailService = new EmailService();
                emailService.sendFromGMail(str, "thay đổi công việc", "thay đổi người thực hiện của công việc :" + ticket.getSubject());
    ...
    public Response updatePriority(
        ...
        try {
            ...
            if (reason_change_priority != null && priority > 0) {
                ...
                System.out.println(str[0]);
                emailService = new EmailService();
                emailService.sendFromGMail(str, "Thay đổi công việc", historyChangePriority + "   " + reason_change_priority);
        ...
        try {
            ...
            if (reason_change_deadline != null && deadline != null) {
                ...
                try {
                    ...
                    String[] str = new String[]{assignto.getEmail()};
                    emailService = new EmailService();
                    emailService.sendFromGMail(str, "thay đổi công việc", ratingstr + reason_change_deadline + "của công việc :" + ticket.getSubject());
...
//            commonBusiness.checkAllConditionToUpdate(userId, ticket_id);
            ...
            if(!ticket.getStatus().equals(Config.STATUS_RESOLVED)){
                return z11.rs.auth.AuthUtil.makeTextResponse(Response.Status.BAD_REQUEST, "Công việc phải resolved mới được đánh giá!");
            }
            /**
             * đánh giá
             */
            if (comment_rating != null && (rating == 1 || rating == 0)) {
                ...
                try {
                    ...
                    String[] str = new String[]{assignto.getEmail()};
                    emailService = new EmailService();
                    emailService.sendFromGMail(str, "thay đổi công việc", ratingstr + comment_rating + "của công việc :" + ticket.getSubject());
                } catch (Exception e) {
    ...
    public Response changeStatusTicket(
            ...
            @FormParam("comment") String commentClose,
            @Context HttpServletRequest request) {
        try {
            ...
            if (statusdesc.equals(Config.STATUS_CLOSED)) {
                if (ticket.getRating() == null) {
                    return z11.rs.auth.AuthUtil.makeTextResponse(Response.Status.BAD_REQUEST, "Yêu cầu đánh giá(hài lòng/không hài lòng) trước khi close công việc!!");
                }
            }
...
//                    ticketThread.setEmployeeId(employee);
//                    ticketThread.setTicketId(ticket);
//                    ticketThread.setContent("Close công việc \n" + commentClose);
//                    ticketThread.setCreatedAt(new Date());
//                    em.persist(ticketThread);
//                }

                Employees assignto = ticket.getAssignedTo();
                String[] str = new String[]{assignto.getEmail()};
                emailService = new EmailService();
                emailService.sendFromGMail(str, "thay đổi công việc", "Thay đổi trạng thái của công việc :" + ticket.getSubject());
                return z11.rs.auth.AuthUtil.makeTextResponse(Response.Status.ACCEPTED, "SUCCESS");
            } else {
